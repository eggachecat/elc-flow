import React from 'react';
import { Card } from 'antd';
import { ItemPanel, Item } from 'gg-editor';
import styles from './index.less';

const RAW_DATA_NODE_SIZE = "250*50"
const OPERATOR_NODE_SIZE = "250*50"
const STATIC_DATA_NODE_SIZE = "250*50"
const MODULE_NODE_SIZE = "250*50"

const FlowItemPanel = () => {
  return (
    <ItemPanel className={styles.itemPanel}>
      <Card bordered={false}>
        <Item
          type="node"
          size={RAW_DATA_NODE_SIZE}
          shape="raw-data-node"
          model={{
            color: '#FA8C16',
            label: '点位',
            _elc_node_type: 'raw_data',
          }}
          src="data:image/svg+xml;base64,"
        />

        <Item
          type="node"
          size={OPERATOR_NODE_SIZE}
          shape="operator-node"
          model={{
            color: '#1890FF',
            label: '算子',
            _elc_node_type: 'operator',
          }}
          src="data:image/svg+xml;base64,"
        />

        <Item
          type="node"
          size={STATIC_DATA_NODE_SIZE}
          shape="static-data-node"
          model={{
            color: '#722ED1',
            label: '变量',
            _elc_node_type: 'static_data',
          }}
          src="data:image/svg+xml;base64,"
        />

        <Item
          type="node"
          size={MODULE_NODE_SIZE}
          shape="module-node"
          model={{
            color: '#13C2C2',
            label: '模组',
            _elc_node_type: 'module',

          }}
          src="data:image/svg+xml;base64,"
        />
      </Card>
    </ItemPanel>
  );
};

export default FlowItemPanel;
